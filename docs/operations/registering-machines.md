...
1. Log in to Omni at [REDACTED](https://REDACTED).
...
!!! warning
    The machine must be able to reach Omni outbound: UDP to the account's WireGuard port, or TCP 443 if using HTTP/2 tunneling. On our network this generally means it needs a route to `REDACTED`.

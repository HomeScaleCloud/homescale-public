...
| Tailscale mesh issue on the sending cluster | Check the `tailscale` app's ingress `ProxyGroup` health on `boa1-prod`, and the sending cluster's dedicated egress proxy pod for the `prometheus-boa1-prod` `ExternalName` Service (`kubectl -n tailscale get pods -l tailscale.com/parent-resource=prometheus-boa1-prod`, hostname `metrics-<cluster>-egress` in the tailnet admin console) |

...
```
Internet → Cloudflare (DNS + proxy) → NetBird reverse proxy → internal xxx service
```

1. A **NetBird reverse proxy** resource is created in Terraform, pointing at the internal `xxx` FQDN. This gives the service an address under `xxx` (the reverse proxy domain registered in `netbird.tf`).
2. A **Cloudflare wildcard CNAME** (`xxx`) resolves to the NetBird reverse proxy cluster. Optionally a second CNAME maps a friendlier public hostname (e.g. `myapp.example.com`) to the same target.

...
`netbird-crs` runs this proxy (`kube-apiserver-proxy`, an `nginx` Deployment in the `netbird` namespace) rather than exposing the cluster's built-in `kubernetes` Service directly. Two things forced that:

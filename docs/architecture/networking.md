...
```
argocd-server.argocd.<cluster>xxx
```
...
- **Region ↔ management connectivity** — bridges region-local services (accessible at `*.<region>xxx`) to the management cluster and vice versa

...
1. A NetBird reverse proxy resource points at the internal `REDACTED` FQDN, giving the service an address under `REDACTED`.
2. A Cloudflare `REDACTED` wildcard CNAME resolves to the NetBird reverse proxy cluster. Optionally a second CNAME is added for a friendlier public URL.

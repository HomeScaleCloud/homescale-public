...
- `local.omni_k8s_grant` — the Omni Kubernetes proxy (`dst: tag:omni-k8s`, Omni's `k8s` Service at `REDACTED`) on `tcp:443`, restricted to `tag:app-automatron` and `group:sg-k8s-admin@REDACTED` only. This endpoint is deliberately **not** covered by omni's `tag:app-omni` policy (which gates the Omni UI/API) — access to the Talos/Omni kube proxy requires either being automatron (the only thing that fetches per-cluster kubeconfigs through it) or activating `sg-k8s-admin` via PIM. A Tailscale-exposed Service reaches this grant by carrying `tag:omni-k8s` (allowed by the kyverno annotation policy alongside `tag:app-*` and `tag:k8s-api`) instead of `tag:app-<name>`.
...
- Three CronJobs, one per playbook, sharing one pod spec (`templates/_pod.tpl`):
  - `automatron-omni-sync` — **the only one that actually fires on its own** (every 5 minutes). Runs `omni-sync.yml`, syncing every `clusters/*/cluster.yaml` and `infra/omni/machineclasses/*.yaml` into Omni. On success (and unless `dry_run` is set), it chains a Job cloned from `automatron-bootstrap-cluster`'s `jobTemplate` — see `CHAIN_NEXT_CRONJOB` in `_pod.tpl`/`entrypoint.sh` — so clusters always exist in Omni before bootstrap-cluster runs against them, without relying on schedule offsets.
...
- Each run's pod: `git-clone` initContainer (`git-sync` image in one-time mode, checks out `main` via SSH) → `tailscale` sidecar (`tag:k8s,tag:app-automatron`, reaches Omni's k8s-proxy and workload-cluster apiservers) → `automatron` container (ansible-core/kubectl/helm/omnictl/envsubst baked into the image; ansible collections installed at start from the git-cloned `requirements.yml`).
- Automatron's own Tailscale OAuth client is dedicated to it, under `/k8s/automatron` (synced via an `InfisicalSecret` CR) — not shared with ArgoCD or any other app. Everything else it needs is deliberately *reused*, not re-provisioned, since identities/service accounts are billed or administratively heavier than plain secrets: its Infisical login reuses the k8s Infisical Operator's own identity (`INFISICAL_OPERATOR_CLIENT_ID`/`_SECRET` at `/k8s/infisical`, synced via a second CR), its Omni access reuses the same Omni service account the `omni` CI job's Plan step still uses (`OMNI_SERVICE_ACCOUNT_KEY` at `/github-actions`, synced via a third CR — that job stays on GitHub Actions too, so it wasn't moved), and `GIT_DEPLOY_KEY` in `/k8s/automatron` is an Infisical secret *reference* pointing at the same value as `/k8s/argocd/deploy-key`'s `sshPrivateKey`, not a separately-minted key.

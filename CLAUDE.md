...
- **`terraform`** (`infra/automatron/job-templates/terraform.yaml`, `scriptPath: infra/automatron/scripts/terraform.sh`, `schedule: "*/15 * * * *"`) plans/applies the single shared `infra/terraform` workspace (state stays in Terraform Cloud — not split per-cluster) from automatron instead of CI, for the same reason `omni-sync` moved: continuous reconciliation on free in-cluster compute instead of only-on-push, billed CI minutes.
  - **Auth**: `provider "infisical"` (`infra/terraform/providers.tf`) picks OIDC (CI's GitHub-federated identity, unchanged) or `universal` auth (`var.infisical_auth_method`) at plan time — there's no GitHub OIDC token available inside a Kubernetes Job for automatron to present. Universal auth reuses the k8s Infisical Operator's own identity (`INFISICAL_OPERATOR_CLIENT_ID`/`_SECRET`, already in automatron's pod env) rather than a dedicated one — same reasoning as automatron's other Infisical use, and why that identity's role is `member` (read/write secrets+folders) rather than `viewer` now (`infra/terraform/modules/infisical/identities.tf`). `CLOUDFLARE_API_TOKEN` and `TF_TOKEN_app_terraform_io` (both otherwise picked up implicitly from the process environment by their respective tools, not through any Terraform variable) and `INFISICAL_ORG_ID` (needed even to authenticate, so can't itself come from Infisical) are synced into `/k8s/automatron` as references to the `/ci` folder's own values (`INFISICAL_ORG_ID` is a literal copy of `var.org_id`, since it isn't stored in Infisical anywhere today) — same `automatron-secrets` `envFrom` every other RGD already has, no new plumbing.
  - **Locking**: `terraform.sh` acquires a `Lease` (`automatron-lock` Role/RoleBinding in `apps/automatron/templates/role.yaml`/`rolebinding.yaml` — generic, not terraform-specific, since other jobs may want the same mechanism later) via atomic `kubectl create` before ever calling `terraform`, retrying with a bounded wait if another run already holds it, and reclaims a stale lock if the Job that's holding it no longer exists. This is deliberate, not redundant with Terraform Cloud's own state lock — TFC's lock only hard-fails a concurrent run outright; this makes scheduled and ad hoc runs (`hsctl run terraform`) queue instead. `JOB_NAME` (a Downward API env var reading the `batch.kubernetes.io/job-name` label the Job controller sets on every Pod — `rgd-jobrun.yaml`/`rgd-jobtemplate.yaml`, generic, not terraform-specific either) is what lets it identify itself as the lock holder and recognize a dead one.

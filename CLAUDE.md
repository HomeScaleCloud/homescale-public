...
- What used to be hand-written Helm `CronJob` templates (`templates/_pod.tpl`'s `include`/`dict` boilerplate) is now a plain CRD interface, backed by [kro](https://kro.run) (`apps/kro`, syncWave -25, `core` only — installs the kro controller + its `ResourceGraphDefinition` CRD). Four `ResourceGraphDefinition`s in `apps/automatron/templates/` (`rgd-jobtemplate.yaml`, `rgd-jobrun.yaml`, `rgd-jobworkflow.yaml`, `rgd-jobworkflowrun.yaml`) define the actual `REDACTED/v1alpha1` CRDs:
  ...
  - **`JobRun`** — a one-off instance of a `JobTemplate`. This is the *only* place a one-off `Job` ever gets created from a template: a human commits one under `infra/automatron/job-runs/`, `hsctl run <template> -e remote` creates one ad hoc, or workflow step-chaining (below) creates one for each step. It resolves its `JobTemplate` via a kro `externalRef` whose lookup name is a CEL expression over the `JobRun`'s own `spec.templateRef` (not a static name) — if this dynamic lookup ever stops resolving, the fallback is having `JobRun` carry its own `playbook`/`scriptPath` fields directly instead of inheriting them.
  - **`JobWorkflow`** — the *template*: an ordered `spec.steps` list of `JobTemplate` refs (each with optional `cluster`/`dryRun` overrides), optionally scheduled. `status.recentRunNames` lists every `JobWorkflowRun` labeled as belonging to it (a pointer index only — see that CRD for detail), aggregated via a label-selector `externalRef` collection.
  - **`JobWorkflowRun`** — one *execution* of a `JobWorkflow`, tracking every step's progress in a central place instead of making callers hunt down scattered `JobRun`/`Job` objects by label. Purely a status-aggregation view — it has no resources of its own to manage; kro projects `status.steps`/`status.phase` from the child `Job`s labeled with this run's name (`REDACTED/workflow-run`) via another label-selector `externalRef` collection and CEL `.map()`/`.exists()`/`.all()`. The `JobWorkflowRun` object itself, and the `JobRun` for each step, are created imperatively (`kubectl apply`, built with `jq -n` rather than heredocs — a heredoc's closing delimiter needs column-0 alignment, which a script embedded in a YAML block scalar inside a Helm template can't reliably guarantee): by the `JobWorkflow` RGD's lightweight "kickoff" container for step 0 of a scheduled run, by `hsctl run <workflow> -e remote` for step 0 of an ad hoc run, or by `entrypoint.sh` for every step after the first. kro's `forEach`/collections can't express "step N waits for step N-1" (collection items are independent), which is why sequencing itself is bash-driven, not kro-managed.
  - The pod boilerplate (serviceAccount, volumes, `git-key-prep`/`git-clone`/`omni-hosts` initContainers, base container) is duplicated across the `JobTemplate`/`JobRun` RGDs (and, at a fraction of the size, the `JobWorkflow` kickoff container) — kro has no Helm-`include`-equivalent for sharing a template fragment across `ResourceGraphDefinition`s. Keep them in sync by hand when changing shared pod shape.
- Instances (the things you actually commit) live under `infra/automatron/`, recursively synced as a 3rd raw source on `clusters/core/apps.yaml`'s `apps-core` Application (same `directory.recurse: true` mechanism `clusters/core/` itself uses): `job-templates/`, `workflows/`, `job-runs/` (rare — most runs are ad hoc), `scripts/`. `.github/scripts/validate-manifests.sh` conforms this tree directly (no Helm rendering involved) alongside every other `directory` source, not just the Helm `apps` catalog. See `docs/operations/hsctl.md`'s "Automatron job CRDs" section for the field reference and examples.
- Migrated defaults: `job-templates/{omni-sync,bootstrap-cluster,bootstrap-core}.yaml` (none scheduled on their own now) and `workflows/omni-sync-and-bootstrap.yaml` (`schedule: "*/15 * * * *"`, steps `[omni-sync, bootstrap-cluster]`) — replaces the old `automatron-omni-sync` CronJob's `CHAIN_NEXT_CRONJOB` hop into `automatron-bootstrap-cluster`.
...
- Workflow chaining (`entrypoint.sh`): when `WORKFLOW_NAME`/`WORKFLOW_RUN_NAME`/`WORKFLOW_STEP_INDEX` env vars are set and the run succeeded, it reads `spec.steps[WORKFLOW_STEP_INDEX+1]` off the named `JobWorkflow` CR and, if present, applies a new `JobRun` for it (carrying `workflowRef`/`workflowRunRef`/`workflowStepIndex`, and labeled `REDACTED/workflow-run` with the same run name, so it keeps showing up under that `JobWorkflowRun`'s aggregated status) — same proven pattern the old single-hop `CHAIN_NEXT_CRONJOB` mechanism used, just walking a list instead of one next-hop. The active-Job dedup guard is scoped to `workflow-run`, not `workflow` — a different run of the same workflow (e.g. an ad hoc run overlapping the scheduled one) is legitimately allowed to be in flight concurrently.
...
- Ad hoc / on-demand runs: `hsctl run <name> -e remote [--dry-run] [--cluster <name>]` (see `docs/operations/hsctl.md`) applies a `JobRun` CR (for a `JobTemplate`) or a `JobWorkflowRun` + step-0 `JobRun` (for a `JobWorkflow`), then streams the resulting `Job`'s logs. `team-infra-plat`/`team-sec-plat` can do this without PIM (`job-operator` ClusterRole in `apps/rbac`, extended with `create`/`get`/`list` on `REDACTED` `jobruns`/`jobworkflowruns` and `get`/`list` on `jobtemplates`/`jobworkflows`, also bound to automatron's own ServiceAccount for its self-chaining and kickoff container); log reading already worked via the existing `pod-operator` binding.
- `deploy.yaml`'s `omni` job: the PR-time dry-run + per-cluster/machineclass comment functionality is unchanged (still calls `omnictl` directly — it's read-only and diff-scoped, not worth routing through automatron). Only the merge-to-`main` **Sync** step changed: it now builds a `core` kubectl context from `CORE_KUBECONFIG` and runs `./hsctl run omni-sync -e remote` followed by `./hsctl run bootstrap-cluster -e remote` (two explicit calls, not the `omni-sync-and-bootstrap` `JobWorkflow` — its own chaining is fire-and-forget, fine for the unattended scheduled run but not for a CI gate that needs to block on and surface both steps' logs), so the actual state-changing sync *and* the bootstrap-cluster run after it happen on automatron with their logs streamed straight into the GitHub Actions log — not executed directly on the runner.

...
- **deploy** — Terraform plan (PR) / apply (main), then Omni plan (PR) / sync (main) for changed clusters, both dispatched to automatron rather than run on the runner — the `omni` job only ever needs `core` cluster access (`CORE_KUBECONFIG`), never Omni network access itself (automatron already has that sorted, in-cluster). **No Tailscale anywhere in CI at all anymore** — the `omni` job was the last user of it (an ephemeral `tailscale/github-action` node, for reaching Omni directly to run `omnictl`) and dropped it once both the plan and sync paths moved to automatron; `terraform` never needed mesh access either, since it only talks to public APIs. Ansible cluster bootstrap no longer runs here at all — see [Automatron](#automatron-ansible-cluster-bootstrap--omni-sync)
